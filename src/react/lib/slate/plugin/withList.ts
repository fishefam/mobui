...
import { BaseOperation, isEditor, Node, Transforms } from 'slate'
import { TObject } from 'type/common'
import { TBlockNode, TNodeType, TSlateEditor } from 'type/slate'

import { isBlockNode } from '..'
import { generateNodeId } from '../util'

export function withList(editor: TSlateEditor) {
  const { apply } = editor
...
export function toggleList(editor: TSlateEditor, type: 'ordered-list' | 'unordered-list') {
  Transforms.setNodes(editor, { listType: type } as TBlockNode)
}
